...
def render_sidebar() -> None:
    ...
    st.sidebar.markdown(
        "This app ingests PDF content, builds a FAISS vector index, and uses Google Gemini 2.5 Flash "
        "for retrieval-augmented responses. Upload a PDF, ask questions, and keep the conversation flowing."
    )
    ...
    st.sidebar.markdown(
        ...
        "- Gemini 2.5 Flash is used only for response generation.\n"
        "- FAISS keeps the most relevant PDF chunks available for retrieval."
    )

    api_key = get_api_key()
    if not api_key:
        st.sidebar.error("Missing GOOGLE_API_KEY environment variable.")
    else:
...
def render_status_cards() -> None:
    ...
    col3.metric("Vector store", vector_status)
...
def main() -> None:
    ...
    st.markdown(
        "Upload a PDF, ask questions, and get citations from the most relevant PDF chunks. "
